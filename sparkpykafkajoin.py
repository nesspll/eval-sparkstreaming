...
customerJSONRedisSchema = StructType([
...
kafkaRedisRawStreamingDF = kafkaRedisRawStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
kafkaRedisRawStreamingDF.withColumn("value",from_json("value",redisTopicSchema))\
    .select(col('value.*'))\
...
zSetDecodedEntriesStreamingDF.withColumn("encodedCustomer", from_json("encodedCustomer", customerJSONRedisSchema))\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
kafkaEventsDF = kafkaRawEventsDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
kafkaEventsDF.withColumn("value", from_json("value", stediEventsSchema))\
    .select(col('value.*'))\
...
stediScoreStreamingDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("email = customer"))
...
stediScoreStreamingDF.selectExpr("cast(email as string) key","to_json(struct(*)) value")\
    .writeStream\
    .format("kafka")\
    .option("kafka.bootstrap.servers", "localhost:9092")\
